          ...
          <div className="mt-2 flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div className="text-sm text-muted-foreground">
                Mostrando {inventoryItems.filter(item => {
                  const totalStock = getTotalStock(item.stock || {});
                  switch (stockFilter) {
                    case 'out': return totalStock === 0;
                    case 'low': return totalStock > 0 && totalStock <= 10;
                    case 'in-stock': return totalStock > 10;
                    default: return true;
                  }
                }).length} de {inventoryItems.length} productos
              </div>
            </div>
            <Button variant="ghost" size="sm" onClick={() => exportCsv(inventoryItems)}>Exportar CSV</Button>
          </div>
        </CardHeader>
        <CardContent>
          <Table>
            ...
            <TableBody>
              {(() => {
                ...
                if (filteredItems.length === 0) {
                  return (
                    <TableRow>
                      <TableCell colSpan={branches.length + 4} className="text-center py-8">
