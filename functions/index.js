...
// body: { accountId, email, password, role }
app.post('/createUserForAccount', async (req, res) => {
  try {
  const { accountId, email, password, role = 'worker' } = req.body;
  ...
  if (!accountId || !email) {
    ...
    await db.doc(`users/${userRecord.uid}`).set({ email, role, accountId, createdAt: Date.now() });
...
app.post('/publicSignup', async (req, res) => {
  try {
    ...
    // create users doc
    await db.doc(`users/${userRecord.uid}`).set({ email, role: 'owner', accountId, displayName: ownerName, createdAt: Date.now() });
